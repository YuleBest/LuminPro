import { ref } from 'vue';
import {
  runCmd, readConfig, writeConfig, updateConfig,
  CONFIG_FILE, BACKUP_CONFIG_FILE, PID_FILE, FLAG_FILE,
...
export function useConfig() {
  ...
  async function save(toast) {
    if (!uiMaxBri.value || !maxBri.value) { toast('亮度値不能为空'); return; }
    toast('保存中...');
  ...
  async function resetToDefaults(toast) {
    ...
    let backup;
    try { backup = JSON.parse(backupRes.stdout); } catch { toast('备份文件损坏'); return; }
