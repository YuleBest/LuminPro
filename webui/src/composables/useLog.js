...
export function useLog() {
  ...
  async function load() {
    ...
    fullLog.value = res.errno === 0 ? (res.stdout.trim() || '暂无日志') : '无法读取日志 (可能模块尚未产生日志文件)';
  }

  async function clear(toast) {
    toast('正在清空日志...');
    const res = await runCmd(`> "${LOG_FILE}"`);
    if (res.errno === 0) { fullLog.value = ''; toast('日志已清空'); }
    else toast('清空失败: ' + (res.stderr || '未知错误'));
  }

  async function copy(toast) {
    const res = await runCmd(`tail -n 50 "${LOG_FILE}"`);
    if (res.errno !== 0 || !res.stdout.trim()) { toast('暂无日志内容'); return; }
    try {
