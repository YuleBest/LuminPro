  ...
  try {
    ...
    if (cmd.includes('cat') && cmd.includes('config.json')) {
      return { errno: 0, stdout: JSON.stringify({
        ui_max_bri: 4095, max_bri: 3500, steps_num: 100, log_max_size: 1024,
        auto_bri_sleep: 1, display_hdr_sleep: 0, sleep_time: '1900-0600',
        inotify_events: 'c',
    ...
    if (cmd.includes('cat') && (cmd.includes('brightness') || cmd.includes('now_bri'))) return { errno: 0, stdout: '1200', stderr: '' };
    if (cmd.includes('cat') && (cmd.includes('max_brightness') || cmd.includes('max_bri'))) return { errno: 0, stdout: '4095', stderr: '' };

    // 日志模拟
    if (cmd.includes('tail') && cmd.includes('service.log')) {
      return { errno: 0, stdout: '[04-07 15:40:01] [service] [INFO    ] LuminPro 服务启动\n[04-07 15:41:05] [up] [INFO    ] 触发提升: 当前亮度 1200 ≥ 阈値 1000\n[04-07 15:42:10] [up] [SUCCESS ] 亮度提升完成 (3500)', stderr: '' };
    }

    // 文档模拟
    if (cmd.includes('cat') && cmd.includes('.md')) return { errno: 0, stdout: '# 调试模式\n\n当前处于 **Web 调试模式**，显示的是模拟测试数据。', stderr: '' };
    if (cmd.includes('cat') && cmd.includes('.txt')) return { errno: 0, stdout: '这是 NOTE.txt 的模拟内容。', stderr: '' };
