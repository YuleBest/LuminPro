...
import {
  ...
  SunMedium, Smartphone, Activity, Clock, Moon, Layers,
...
async function onSliderChange(e) {
  ...
  if (pct < 5 && !confirmedLowBri.value) {
    ...
    if (!ok) { const safe = Math.ceil(sysMax * 0.05); e.target.value = safe; return; }
...
const statusItems = computed(() => [
  ...
  { label: 'inotifyd PID', value: status.inotifydPid.value, icon: Activity },
  { label: 'inotifyd 状态', value: status.inotifydState.value, icon: Clock },
