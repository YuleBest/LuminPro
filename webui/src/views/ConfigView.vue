...
import { Save, Settings2, ShieldCheck, FileText, Settings, Monitor, HelpCircle } from 'lucide-vue-next';
...
function handleReset() {
  if (!resetConfirming.value) {
    ...
    resetTimer = setTimeout(() => { resetConfirming.value = false; }, 5000);
    return;
  }
  ...
  <section class="card" id="config-section" data-nav-group="config">
    ...
    <div class="config-group">
      ...
      <div class="config-item">
        ...
        <Input v-model="config.stepsNum.value" type="number" min="1" max="500" class="config-input" />
      </div>
    </div>
    ...
    <div class="config-group">
      ...
      <div class="config-item">
        <div class="config-label">
          ...
          <span class="config-desc" style="color:var(--color-warning)">建议配合去温控使用。出现闪屏时可改用黑名单功能</span>
        </div>
      ...
      <div class="config-item-expand" :class="{ show: config.sleepMode.value }">
        <div class="custom-time-picker">
          <div class="time-unit">
            <input type="number" v-model="config.sleepStartH.value" class="time-input" min="0" max="23" placeholder="00" />
            <span>:</span>
            <input type="number" v-model="config.sleepStartM.value" class="time-input" min="0" max="59" placeholder="00" />
          </div>
          <span class="time-sep">至</span>
          <div class="time-unit">
            <input type="number" v-model="config.sleepEndH.value" class="time-input" min="0" max="23" placeholder="00" />
            <span>:</span>
            <input type="number" v-model="config.sleepEndM.value" class="time-input" min="0" max="59" placeholder="00" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="config-group">
      ...
      <div class="config-item">
        ...
        <Input v-model="config.logMaxSize.value" type="number" min="1" max="10240" class="config-input" />
      </div>
    </div>

    <div class="card-actions">
      <Button variant="outline" @click="handleReset" :class="resetConfirming ? 'border-destructive text-destructive' : ''">
        {{ resetConfirming ? '确认恢复？' : '恢复默认' }}
      </Button>
      <Button @click="config.save(showToast)">
        <Save :size="15" /> 保存配置
      </Button>
    </div>
  </section>
    ...
    <div class="config-group">
      ...
      <div class="config-item">
        <div class="config-label">
          ...
          <span class="config-desc" style="color:var(--color-warning)">修改为 c 以外时建议关闭自动刷新</span>
          <button class="btn-text-s" style="margin-top:6px;padding:2px 6px" @click="viewInotifyHelp">
            查看支持事件
          </button>
        </div>
        <Input v-model="config.inotifyEvents.value" type="text" placeholder="c" class="config-input" />
      </div>
  ...
  <section class="card" id="webui-config-section" data-nav-group="config">
    ...
    <div class="config-group">
      ...
      <div class="config-item" v-show="config.autoRefresh.value">
        ...
        <Input v-model="config.statusRefreshInterval.value" type="number" min="100" class="config-input" />
      </div>
      ...
      <div class="config-item">
        ...
        <div class="theme-seg">
          <button
            v-for="opt in [{v:'system',l:'系统'},{v:'light',l:'浅色'},{v:'dark',l:'深色'}]"
            :key="opt.v"
            class="theme-seg-btn"
            :class="{ active: config.themeMode.value === opt.v }"
            @click="config.themeMode.value = opt.v; config.applyTheme(opt.v)"
          >{{ opt.l }}</button>
        </div>
      </div>
      ...
      <div class="config-item">
        ...
        <Input v-model="config.uiZoom.value" type="number" min="50" max="150" class="config-input" />
      </div>
    </div>

    <div class="card-actions">
      <Button @click="handleSaveWebUI">
        <Save :size="15" /> 保存配置
      </Button>
    </div>
  </section>
