...
function handleClear() {
  if (!clearConfirming.value) {
    ...
    clearConfirmTimer = setTimeout(() => { clearConfirming.value = false; }, 3000);
    return;
  }
    ...
    <div class="card-header" style="margin-bottom:8px">
      ...
      <div class="header-actions dropdown-container">
        ...
        <div class="dropdown-menu" :class="{ show: logMenuOpen }" @click.stop>
          <div class="dropdown-item" @click="log.copy(showToast); logMenuOpen=false">
            <Copy :size="18" /><span>复制日志</span>
          </div>
          <div class="dropdown-item" @click="log.exportLog(showToast); logMenuOpen=false">
            <Download :size="18" /><span>导出日志文件</span>
          </div>
          <div class="dropdown-item" :class="{ 'dropdown-item-danger': clearConfirming }" @click="handleClear">
            <Trash2 :size="18" />
