...
export function setupAppsEvents() {
    ...
    btnSmartSelect.addEventListener('click', () => {
      ...
      items.forEach(item => {
        ...
        for (const kw of smartKeywords) {
          if (kw === 'x') {
            if (name === 'x' || name === 'x (twitter)') { match = true; break; }
  ...
  if (btnSelectAll) {
    btnSelectAll.addEventListener('click', () => {
      document.querySelectorAll('.app-checkbox').forEach(cb => { cb.checked = true; });
      updateUnsavedStyling();
      appsDropdownMenu && appsDropdownMenu.classList.remove('show');
    });
  }
  ...
  if (btnInvertSelection) {
    btnInvertSelection.addEventListener('click', () => {
      document.querySelectorAll('.app-checkbox').forEach(cb => { cb.checked = !cb.checked; });
      updateUnsavedStyling();
      appsDropdownMenu && appsDropdownMenu.classList.remove('show');
    });
  }
  ...
  if (btnAppsRefresh) {
    btnAppsRefresh.addEventListener('click', () => { reorderDOMApps(); });
  }
